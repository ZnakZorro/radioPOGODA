...
void audio_setStationByStringNumber(const String ParamValue){
    int nr = ParamValue.toInt();
    Serial.println(nr);
    Serial.println(lastStation);
    if (nr<0) nr=0;
    if (nr>lastStation) nr=lastStation; 
    Serial.println(nr);   
...
int getlastStation(){
    ...
    Serial.print("lastStation=");Serial.println(lastStation); 
    return lastStation;   
}

void saveRadioStreams(const String ParamValue){
    zapiszSPIFFS("/streams.csv",ParamValue.c_str());
    lastStation = getlastStation();
    /*    
    String l0 = clio.splitValue(odczyt, '\n',0);
    String l0v0 = clio.splitValue(l0, ';',0);
    String l0v1 = clio.splitValue(l0, ';',1);
    Serial.println(l0);
    Serial.println(l0v0);
    Serial.println(l0v1);
    String l1 = clio.splitValue(odczyt, '\n',1);
    Serial.println(l1);
    String l2 = clio.splitValue(odczyt, '\n',2);
    Serial.println(l2);
    */
...
void installServer(){
  ...
  server.on("/streams.csv", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(SPIFFS, "/streams.csv", "text/csv");
    //request->send(200, "application/json", odczytajSPIFFS("/rrr.json"));
  }); 
    ...
    /******************************/
     ...
     for(int i=0;i<params;i++){
          ...
          String ParamValue = p->value();
           ...
           if (ParamName=="nr")  audio_setStationByStringNumber(ParamValue);
