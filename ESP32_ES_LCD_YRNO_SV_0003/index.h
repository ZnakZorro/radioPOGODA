...
.col-bg{grid-template-columns:repeat(auto-fit,minmax(120px,1fr));}
...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  fetch(url+"streams.csv")
  .then(j => {return j.text()})
  .then(csv => {
    //rrrstacje(s);
    stacjeARR = csv.split("\n");
    renderstacje(stacjeARR);
    //console.log(stacjeARR);    
  }).catch(e => {console.log(e)})
...
let renderstacje=(st)=>{
  ...
  st.forEach((s,i)=>{
    let [k,v] = s.split(";");
    //console.log(i,k,v)
    if (k&&v) {h+='<button class="btn-auto" id="sta'+i+'" data-n="'+i+'" data-s="'+v+'" onClick="sta(this)">'+k+'</button>';}
  });
...
let sta=(ten)=>{
  console.log(ten.dataset.n,ten.dataset.s,ten.textContent);
  sn("radio?nr="+ten.dataset.n);
...
<div class="wrap">
  ...
  <div id="streams" class="hidden">
      ...
      <div class="grid col col-bg st stu" id="stacje"></div>
...
const usun=(jj)=>{
  if (confirm("Usuwasz "+jj+" :: "+stacjeARR[jj]) == true) {
      console.log("jj=",jj); 
      delete stacjeARR[jj];
      opiszLocaleModal();
  }
...
const DodajStream=()=>{
  let nr = stacjeARR.length+1;
  stacjeARR.push("nowy-"+nr+"; ");
  opiszLocaleModal();
}
