...
String ESPio::payload2LCD(String payload){
  ...
  minuteStr.trim();
  minuteStamp = (long) minuteStr.toInt();
  minuteZero = millis();
      //long unsigned int aa = strtoul(minuteStr);//, 0, 16);
      unsigned long bb     = atol(minuteStr.c_str());
...
String ESPio::getClock(){         
    unsigned long minutFromMillis = (millis()-minuteZero)/(60*1000);
